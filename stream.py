...
st.markdown(
    f'<p style="background-color: #262730; color: white ; font-size: 40px; font-weight: bold; text-align:">Умный поиск фильмов</p>',
...
def display_movie(i):
    ...
    imges = df["img_url"][i]
    genre_movie = (
        "Нет данных" if pd.isna(df["genres"].iloc[i]) else df["genres"][i].title()
    )
...
if len(df) != 0:
    ...
    if len_title < min_svd and len(df) > 10000:
    ...
    if st.button("Подобрать фильм"):
        if len_title < min_svd and len(df) > 10000:
            ...
            for i in similar_movies:
                if i == -1:
                    st.title(
                        "Больше рекомендаций нет, уменьшите количество рекомендаций или измените фильтры"
                    )
                    break
                else:
                    display_movie(i)
    ...
    if st.button("Выбрать случайно"):
        ...
        if size == 0:
            st.header(
                "Невозможно сделать случайный выбор, т.к по таким параметрам фильмы не найдены"
            )
